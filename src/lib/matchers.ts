...
import { OpenmailSDKDefaultError } from "../models/errors/openmail-sdk-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | OpenmailSDKDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | OpenmailSDKDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new OpenmailSDKDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new OpenmailSDKDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
